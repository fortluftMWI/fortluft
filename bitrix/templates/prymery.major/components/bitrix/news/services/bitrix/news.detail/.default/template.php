<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();
/** @var array $arParams */
/** @var array $arResult */
/** @global CMain $APPLICATION */
/** @global CUser $USER */
/** @global CDatabase $DB */
/** @var CBitrixComponentTemplate $this */
/** @var string $templateName */
/** @var string $templateFile */
/** @var string $templateFolder */
/** @var string $componentPath */
/** @var CBitrixComponent $component */
$this->setFrameMode(true);
PRmajor::pre($arResult);


?>


<div class="container post-detail page_simple">
	<div class="service-detail__head">
		<div class="row">
			<div class="col-12">
				<div class="services-detail__content">
					<div class="service-detail__thumb">
						<svg class="icon"><use xlink:href="#contract"></use></svg>
					</div>
					<div class="servives-detail__info">
						<div class="services-detail__description">
							<?=$arResult['~PREVIEW_TEXT']?>
						</div>
						<div class="services-detail__price">
							<?=$arResult['MIN_PRICE']['VALUE_FORMATED']?>
						</div>
					</div>
					<div class="services-detail__action">
						<a href="javascript:void(0)" class="adp-btn adp-btn-primary"><?=GetMessage('PRYMERY_ORDER_BTN');?></a>
						<?if($arResult['PROPERTIES']['HELPFUL']['VALUE']):?>
							<div class="services-detail__tip">
								<div class="header">
									<button class="adp-btn btn-tip">?</button>
								</div>
								<div class="tip">
									<div class="tip__inner">
										<div class="tip__content">
											<?=$arResult['PROPERTIES']['HELPFUL']['~VALUE']?>
										</div>
									</div>
								</div>
							</div>
						<?endif;?>
					</div>
				</div>
			</div>
		</div>
	</div>

	<div class="row">
		<div class="col-12 col-md-6">
			<h3>В перечень услуги входит:</h3>

			<ul class="services-included">
				<li>Подбор необходимой упаковки</li>
				<li>Защитные мероприятия при упаковке товара</li>
				<li>Тара от поставщика по самим низким ценам</li>
				<li>маленькие сроки проведения услуг</li>
			</ul>
		</div>

		<div class="col-12 col-md-6">
			<h3>В перечень услуги не входит:</h3>

			<ul class="services-excluded">
				<li>Проверка срока годности товара</li>
				<li>Подбор габаритов тары для упаковки</li>
				<li>Расчеты прочности упаковки для конкретного товара</li>
			</ul>
		</div>
	</div>

	<h3>Частые вопросы по услуге:</h3>

	<div class="slide-toggle">
		<div class="header">Почему отличаются реальные цены и цены указанные в прейскуранте?</div>
		<div class="body">
			Цены обусловлены габаритами заготовки для будущей упаковки. Размеры напрямую зависят от выбранной конструкции. Чем она сложнее, тем затратнее. Нужно помнить, что при раскрое остаются обрезки сырья.
		</div>
	</div>

	<div class="slide-toggle">
		<div class="header">Каковы сроки упаковки продукции?</div>
		<div class="body">
			Цены обусловлены габаритами заготовки для будущей упаковки. Размеры напрямую зависят от выбранной конструкции. Чем она сложнее, тем затратнее. Нужно помнить, что при раскрое остаются обрезки сырья.
		</div>
	</div>

	<div class="slide-toggle">
		<div class="header">Какое максимальное количество можно упаковать за один заказ?</div>
		<div class="body">
			Цены обусловлены габаритами заготовки для будущей упаковки. Размеры напрямую зависят от выбранной конструкции. Чем она сложнее, тем затратнее. Нужно помнить, что при раскрое остаются обрезки сырья.
		</div>
	</div>

	<div class="slide-toggle">
		<div class="header">Существует ли возможность заказать комплекс услуг?</div>
		<div class="body">
			Цены обусловлены габаритами заготовки для будущей упаковки. Размеры напрямую зависят от выбранной конструкции. Чем она сложнее, тем затратнее. Нужно помнить, что при раскрое остаются обрезки сырья.
		</div>
	</div>
</div>

<div class="services-contacts">
	<div class="container">
		<div class="full-contacts">
			<div class="row justify-content-center">
				<div class="col-12 col-sm-6 col-lg-4">
					<div class="main-contact__item">
						<div class="main-contact__thumb">
							<svg class="main-contact__icon" viewBox="0 0 128 128"><g><path d="m78.777 37.021a14.777 14.777 0 1 0 -14.777 14.779 14.795 14.795 0 0 0 14.777-14.779zm-26.054 0a11.277 11.277 0 1 1 11.277 11.279 11.29 11.29 0 0 1 -11.277-11.279z"/><path d="m123.328 121.069-14.266-37.4a1.751 1.751 0 0 0 -1.635-1.126h-27c.165-.269.329-.53.494-.8 10.389-17.2 15.617-32.246 15.542-44.714a32.464 32.464 0 0 0 -64.928-.011c-.075 12.479 5.153 27.527 15.542 44.725.165.273.329.534.494.8h-27a1.751 1.751 0 0 0 -1.635 1.126l-14.264 37.4a1.748 1.748 0 0 0 1.635 2.374h115.386a1.748 1.748 0 0 0 1.635-2.374zm-88.292-84.048a28.964 28.964 0 1 1 57.928.01c.15 24.858-23.09 55.517-28.964 62.869-5.874-7.349-29.115-38-28.964-62.879zm27.631 66.779a1.75 1.75 0 0 0 2.666 0 185.716 185.716 0 0 0 12.9-17.759h27.987l2.24 5.875-54.691 19.451-19.494-25.329h15.49a185.716 185.716 0 0 0 12.902 17.762zm-8.959 11.3h.01l32.627-11.6 12.655 16.443h-58.9zm-31.93-29.062h8.08l20.442 26.562-20.643 7.342h-20.81zm81.643 33.905-13.609-17.682 19.9-7.077 9.443 24.759z"/></g></svg>
						</div>

						<div class="main-contact__content">
							<div class="title">г. Москва, Центральный&nbsp;офис</div>
							<div class="description">ул. Черняховского, 86, корпус 8</div>
						</div>
					</div>
				</div>

				<div class="col-12 col-sm-6 col-lg-4">
					<div class="main-contact__item">
						<div class="main-contact__thumb">
							<svg class="main-contact__icon" viewBox="0 0 512 512" ><path d="m459.164 2.196c-2.015-2.014-4.991-2.71-7.691-1.807l-273.051 91.68c-3.926 1.319-6.041 5.571-4.723 9.498 1.318 3.926 5.567 6.04 9.498 4.723l222.541-74.721-263.854 181.927-69.881-69.879 78.015-26.198c3.927-1.318 6.041-5.57 4.722-9.497-1.318-3.927-5.567-6.044-9.497-4.722l-89.49 30.051c-2.461.826-4.32 2.867-4.914 5.395-.593 2.527.162 5.183 1.998 7.019l80.855 80.854 5.326 85.218-1.849 1.85c-2.929 2.93-2.929 7.678.001 10.606 1.465 1.464 3.384 2.196 5.303 2.196 1.92 0 3.84-.732 5.304-2.197l1.849-1.849 85.217 5.326 80.856 80.855c1.423 1.423 3.338 2.196 5.304 2.196.571 0 1.146-.065 1.715-.199 2.528-.594 4.568-2.452 5.396-4.914l61.42-182.929c1.318-3.927-.797-8.18-4.723-9.498-3.926-1.317-8.18.795-9.498 4.724l-57.566 171.453-69.88-69.88 181.928-263.864-43.346 129.11c-1.318 3.927.797 8.179 4.724 9.497.792.266 1.596.392 2.388.392 3.134 0 6.057-1.98 7.109-5.115l60.3-179.611c.906-2.698.207-5.676-1.806-7.69zm-224.788 310.414-70.607-4.413 237.196-237.196zm155.98-252.215-237.195 237.194-4.412-70.606z"/><path d="m121.392 399.014h-12.783c-4.143 0-7.5 3.357-7.5 7.5s3.357 7.5 7.5 7.5h12.783c4.143 0 7.5-3.357 7.5-7.5s-3.357-7.5-7.5-7.5z"/><path d="m186.239 425.349c-3.962-1.188-8.148 1.055-9.342 5.023-.593 1.971-1.686 3.763-3.25 5.326l-1.964 1.964c-2.929 2.93-2.929 7.678 0 10.607 1.465 1.464 3.385 2.196 5.304 2.196s3.839-.732 5.304-2.196l1.964-1.964c3.336-3.336 5.693-7.244 7.008-11.615 1.192-3.965-1.057-8.147-5.024-9.341z"/><path d="m89.473 370.082c-3.619-2.01-8.188-.704-10.197 2.918-2.226 4.009-3.401 8.562-3.401 13.166 0 1.289.092 2.594.272 3.876.529 3.746 3.74 6.452 7.417 6.452.35 0 .703-.024 1.059-.074 4.102-.58 6.957-4.374 6.378-8.476-.084-.59-.126-1.188-.126-1.778 0-2.093.51-4.073 1.517-5.887 2.009-3.621.703-8.188-2.919-10.197z"/><path d="m109.328 359.531c1.919 0 3.839-.732 5.304-2.196l9.039-9.039c2.929-2.93 2.929-7.678 0-10.607-2.93-2.928-7.678-2.928-10.607 0l-9.039 9.039c-2.929 2.93-2.929 7.678 0 10.607 1.464 1.463 3.384 2.196 5.303 2.196z"/><path d="m175.637 408.077c.698-4.083-2.046-7.959-6.129-8.656-1.576-.27-3.206-.406-4.843-.406h-9.186c-4.143 0-7.5 3.357-7.5 7.5s3.357 7.5 7.5 7.5h9.186c.793 0 1.571.064 2.315.191.428.073.853.108 1.272.108 3.588 0 6.76-2.582 7.385-6.237z"/><path d="m147.581 461.766-9.039 9.039c-2.929 2.93-2.929 7.678 0 10.607 1.465 1.464 3.385 2.196 5.304 2.196s3.839-.732 5.304-2.196l9.039-9.039c2.929-2.93 2.929-7.678 0-10.607-2.93-2.928-7.678-2.928-10.608 0z"/><path d="m114.393 494.954-4.242 4.242c-2.929 2.93-2.929 7.678 0 10.607 1.465 1.464 3.385 2.196 5.304 2.196s3.839-.732 5.304-2.196l4.242-4.242c2.929-2.93 2.929-7.678 0-10.607-2.93-2.928-7.678-2.928-10.608 0z"/></svg>
						</div>

						<div class="main-contact__content">
							<div class="title"><a href="mailto:info@prymery.com">info@prymery.com</a></div>
							<div class="description">онлайн поддержка</div>
						</div>
					</div>
				</div>

				<div class="col-12 col-sm-6 col-lg-4">
					<div class="main-contact__item">
						<div class="main-contact__thumb">
							<svg viewBox="0 0 512 512"><path d="m387.312 58.573h-145.92c-4.234 0-7.665 3.432-7.665 7.665s3.431 7.665 7.665 7.665h145.92c4.234 0 7.665-3.432 7.665-7.665s-3.431-7.665-7.665-7.665z"/><path d="m387.312 94.34h-145.92c-4.234 0-7.665 3.432-7.665 7.665s3.431 7.665 7.665 7.665h145.92c4.234 0 7.665-3.432 7.665-7.665 0-4.234-3.431-7.665-7.665-7.665z"/><path d="m477.865 138.142h-1.018v-18.355c0-12.219-9.941-22.16-22.16-22.16h-7.286v-13.472c0-4.233-3.431-7.665-7.665-7.665s-7.665 3.432-7.665 7.665v53.987h-235.437v-116.725c0-3.357 2.731-6.087 6.087-6.087h223.263c3.357 0 6.088 2.731 6.088 6.087v26.905c0 4.233 3.431 7.665 7.665 7.665s7.665-3.432 7.665-7.665v-26.905c-.001-11.81-9.609-21.417-21.418-21.417h-223.263c-11.809 0-21.416 9.607-21.416 21.417v76.211h-7.287c-8.753 0-16.335 5.102-19.934 12.488-6.929-6.191-16.062-9.963-26.063-9.963h-51.951c-21.201 0-38.51 16.937-39.136 37.99h-2.799c-18.445 0-33.451 15.006-33.451 33.451v268.967c0 18.445 15.006 33.451 33.451 33.451h2.799c.626 21.051 17.935 37.988 39.136 37.988h51.95c21.202 0 38.511-16.937 39.137-37.99h310.709c18.445 0 33.451-15.006 33.451-33.451v-268.967c-.001-18.444-15.007-33.45-33.452-33.45zm-30.464-25.185h7.286c3.766 0 6.83 3.064 6.83 6.83v18.355h-14.117v-25.185zm-273.384 0h7.287v25.185h-14.118v-18.355c.001-3.767 3.065-6.83 6.831-6.83zm-158.004 327.602v-268.967c0-9.992 8.13-18.121 18.122-18.121h2.769v14.765c0 35.81 4.911 58.652 7.431 80.084v115.26c-9.671 47.91-6.965 81.911-7.431 95.1h-2.769c-9.992.001-18.122-8.129-18.122-18.121zm135.844 32.275c0 13.144-10.694 23.837-23.837 23.837h-51.95c-13.144 0-23.836-10.693-23.836-23.837.712-24.236-2.767-57.726 7.276-106.951.103-.504.155-1.018.155-1.532v-116.55c0-.345-.023-.691-.07-1.033-2.506-21.219-7.361-43.883-7.361-78.532v-28.918c0-13.144 10.693-23.837 23.836-23.837h51.95c13.144 0 23.837 10.693 23.837 23.837v6.489 22.43c0 34.514-4.76 56.502-7.361 78.532-.047.342-.07.688-.07 1.033v116.548c0 .514.052 1.028.155 1.532 10.035 49.194 6.558 82.51 7.276 106.952zm344.13-32.275c0 9.993-8.13 18.122-18.122 18.122h-310.678c-.444-12.542 2.249-47.134-7.431-95.1v-115.261c2.533-21.543 7.431-44.157 7.431-80.084v-14.765h310.679c9.992 0 18.122 8.129 18.122 18.121v268.967z"/><path d="m417.381 187.72h-206.057c-9.83 0-17.828 7.997-17.828 17.828v48.08c0 9.83 7.998 17.828 17.828 17.828h35.831c4.234 0 7.665-3.432 7.665-7.665s-3.431-7.665-7.665-7.665h-35.831c-1.378 0-2.499-1.121-2.499-2.498v-48.08c0-1.378 1.121-2.498 2.499-2.498h206.057c1.377 0 2.498 1.121 2.498 2.498v48.08c0 1.378-1.121 2.498-2.498 2.498h-134.392c-4.234 0-7.665 3.432-7.665 7.665s3.431 7.665 7.665 7.665h134.392c9.83 0 17.827-7.997 17.827-17.828v-48.08c.001-9.83-7.996-17.828-17.827-17.828z"/><path d="m228.573 296.387h-20.172c-8.219 0-14.905 6.686-14.905 14.905v19.411c0 8.219 6.687 14.905 14.905 14.905h20.172c8.219 0 14.904-6.687 14.904-14.905v-19.411c0-8.219-6.686-14.905-14.904-14.905zm-.425 33.891h-19.322v-18.562h19.322z"/><path d="m292.483 296.387h-20.172c-8.219 0-14.905 6.686-14.905 14.905v19.411c0 8.219 6.687 14.905 14.905 14.905h20.172c8.219 0 14.905-6.687 14.905-14.905v-19.411c0-8.219-6.686-14.905-14.905-14.905zm-.424 33.891h-19.323v-18.562h19.323z"/><path d="m356.393 296.387h-20.172c-8.219 0-14.905 6.686-14.905 14.905v19.411c0 8.219 6.687 14.905 14.905 14.905h20.172c8.219 0 14.905-6.687 14.905-14.905v-19.411c0-8.219-6.686-14.905-14.905-14.905zm-.424 33.891h-19.323v-18.562h19.323z"/><path d="m420.303 296.387h-20.172c-8.219 0-14.904 6.686-14.904 14.905v19.411c0 8.219 6.686 14.905 14.904 14.905h20.172c8.219 0 14.905-6.687 14.905-14.905v-19.411c.001-8.219-6.686-14.905-14.905-14.905zm-.424 33.891h-19.322v-18.562h19.322z"/><path d="m228.573 369.27h-20.172c-8.219 0-14.905 6.686-14.905 14.905v19.411c0 8.219 6.687 14.905 14.905 14.905h20.172c8.219 0 14.904-6.686 14.904-14.905v-19.411c0-8.218-6.686-14.905-14.904-14.905zm-.425 33.892h-19.322v-18.562h19.322z"/><path d="m292.483 369.27h-20.172c-8.219 0-14.905 6.686-14.905 14.905v19.411c0 8.219 6.687 14.905 14.905 14.905h20.172c8.219 0 14.905-6.686 14.905-14.905v-19.411c0-8.218-6.686-14.905-14.905-14.905zm-.424 33.892h-19.323v-18.562h19.323z"/><path d="m356.393 369.27h-20.172c-8.219 0-14.905 6.686-14.905 14.905v19.411c0 8.219 6.687 14.905 14.905 14.905h20.172c8.219 0 14.905-6.686 14.905-14.905v-19.411c0-8.218-6.686-14.905-14.905-14.905zm-.424 33.892h-19.323v-18.562h19.323z"/><path d="m420.303 369.27h-20.172c-8.219 0-14.904 6.686-14.904 14.905v19.411c0 8.219 6.686 14.905 14.904 14.905h20.172c8.219 0 14.905-6.686 14.905-14.905v-19.411c.001-8.218-6.686-14.905-14.905-14.905zm-.424 33.892h-19.322v-18.562h19.322z"/></svg>
						</div>

						<div class="main-contact__content">
							<div class="title"><a href="tel:+74992131003">+7 499 213 10 03</a></div>
							<div class="description">Пн - Пт, 10:30 - 17:00</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>

<div class="services-callback">
	<div class="container">
		<div class="main-callback">
			<form action="#" method="POST">
				<div class="row">
					<div class="col-12">
						<div class="main-callback__title">Остались вопросы?</div>
						<div class="main-callback_description">Свяжитесь с нами, используя форму, и мы перезвонм Вам в ближайшее время.</div>
					</div>

					<div class="col-12 col-md-6 col-lg-6">
						<div class="form-group">
							<input type="text" name="name" class="form-control" placeholder="Ваше имя">
						</div>
					</div>

					<div class="col-12 col-md-6 col-lg-6">
						<div class="form-group">
							<input type="text" name="phone" class="form-control" placeholder="Ваш телефон" required>
						</div>
					</div>

					<div class="col-12">
						<div class="form-group">
							<textarea name="message" rows="6" class="form-control" placeholder="Ваше сообщение"></textarea>
						</div>
					</div>

					<div class="col-12">
						<input type="submit" class="btn-submit adp-btn adp-btn-primary" value="Задать вопрос">
					</div>
				</div>
			</form>
		</div>
	</div>
</div>
