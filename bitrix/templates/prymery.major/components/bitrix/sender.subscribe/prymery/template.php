...
<div class="bx-subscribe"  id="sender-subscribe">
    ...
    <?if(isset($arResult['MESSAGE'])): CJSCore::Init(array("popup"));?>
        <div id="sender-subscribe-response-cont" style="display: none;">
            <div class="bx_subscribe_response_container">
                <table>
                    <tr>
                        <td style="padding-right: 40px; padding-bottom: 0px;"></td>
                        <td>
    ...
    <form id="bx_subscribe_subform_<?=$buttonId?>" role="form" method="post" class="form form-subscribe" action="<?=$arResult["FORM_ACTION"]?>">
        <?=bitrix_sessid_post()?>
		...
		<div class="form-group">
			...
			<svg class="icon-input"><use xlink:href="#at"></use></svg>
		</div>
        ...
        <?endif;?>
		<button class="adp-btn adp-btn-primary" id="bx_subscribe_btn_<?=$buttonId?>">
    ...
    <button type="submit" class="adp-btn adp-btn--danger">
        <svg class="icon icon-md"><use xlink:href="#long-arrow-right"></use></svg>
    </button>
    <form id="bx_subscribe_subform_<?=$buttonId?>" role="form" class="form form__mailing" method="post" action="<?=$arResult["FORM_ACTION"]?>">
        <?=bitrix_sessid_post()?>
        <input type="hidden" name="sender_subscription" value="add">
        <input class="form-control" type="email" name="SENDER_SUBSCRIBE_EMAIL" value="" title="<?=GetMessage("subscr_form_email_title")?>" placeholder="<?=htmlspecialcharsbx(GetMessage('subscr_form_email_title'))?>">
        <div style="<?=($arParams['HIDE_MAILINGS'] <> 'Y' ? '' : 'display: none;')?>">
            <?if(count($arResult["RUBRICS"])>0):?>
                <div class="bx-subscribe-desc"><?=GetMessage("subscr_form_title_desc")?></div>
            <?endif;?>
        ...
        <div class="bx_subscribe_submit_container">
            <button class="adp-btn adp-btn--primary adp-btn--icon-right" id="bx_subscribe_btn_<?=$buttonId?>">
                <?=GetMessage('SUBSCRIBE_BTN_TEXT');?>
                <img src="<?=SITE_TEMPLATE_PATH?>/assets/img/icons/arrow-right-long-white.png" alt="-->">
            </button>
        </div>
    </form>
    <?$frame->end();?>
