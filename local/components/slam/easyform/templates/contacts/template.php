    ...
    <form id="<?=$FORM_ID?>"
        ...
        <div class="row" >
            ...
            if(!empty($arResult['FORM_FIELDS'])):
                foreach($arResult['FORM_FIELDS'] as $fieldCode => $arField):
                    $i++;
                    if ($i ==4){echo "</div><div style='col-md-6'>";}
                    ...
                    if($arField['TYPE'] == 'textarea'):?>
                        <div class="col-xs-12 <? if ($i>3){echo 'col-md-6';}?>">
                            <div class="form-group">
                                <? if(!$arParams['HIDE_FIELD_NAME']): ?>
                                    <label class="control-label" for="<?=$arField['ID']?>"><?=$arField['TITLE']?></label>
                                <? endif; ?>
                                <div>
                                    <textarea class="form-control" id="<?=$arField['ID']?>" rows="5" name="<?=$arField['NAME']?>" <?=$arField['REQ_STR']?>><?=$arField['VALUE'];?></textarea>
                                </div>
                            </div>
                        </div>
                    ...
                    <?elseif($arField['TYPE'] == 'select'):?>
                        <div class="col-xs-12 switch-select <? if ($i>3){echo 'col-md-6';}?>"">
                        ...
                        <div class="col-xs-12 <? if ($i>3){echo 'col-md-6';}?>"">
                            <div class="form-group">
                                <? if(!$arParams['HIDE_FIELD_NAME']): ?>
                                    <label class="control-label" for="<?=$arField['ID']?>"><?=$arField['TITLE']?></label>
                                <? endif; ?>
                                <input class="form-control" type="<?=$arField['TYPE'];?>" id="<?=$arField['ID']?>" name="<?=$arField['NAME']?>" value="<?=$arField['VALUE'];?>" <?=$arField['REQ_STR']?> <?=$arField['MASK_STR']?>>
                            </div>
                        </div>
                    <?endif;
                endforeach;?>
                ...
                onClick="yaCounter29142655.reachGoal('<?php $uri = $_SERVER['REQUEST_URI'];
                    if($uri == '/dealer/' or $uri == '/') {echo "dealer";}
                    else {echo "contacts";}
                    ?>'); return true;" data-default="<?=$arParams['FORM_SUBMIT_VALUE']?>"><?=$arParams['FORM_SUBMIT_VALUE']?></button>
                </div>
            <?endif;?>
