...
$arPatterns = getPatterns1($q);
function getPatterns1($sSearch) 
{
	$arPatterns = "|(?<!</)(?<!<)($sSearch)+(?!>)|iu";
	
	return $arPatterns;
}
function checkCrossNumber1($arItemProps, $sWord) 
{
		...
		/*if(in_array((string)$sWord, $arItemProps))
		{
			return true;
		}*/
	return false;
...
if(isset($_REQUEST["tags"]))
	...
	$tags = false;
if(isset($_REQUEST["q"])) // edited here:
{
	$q = trim($_REQUEST["q"]);
	//$q = str_replace([".","-", "_", "/", ":",], "", $q);
	$query = str_replace([".","/", ":"," ",",","-","_",], "", $q);
	AddMessage2Log($q,'QUERY');
	AddMessage2Log($query,'REPLACEQUERY');
}
else
	$q = false;
...
if($this->InitComponentTemplate($templatePage))
{
	...
	if($folderPath <> '')
	{
		...
		$res = CIBlockElement::GetList(Array(), array("IBLOCK_ID"=>16,"ACTIVE"=>'Y',"PROPERTY_CROSS_NUMBERS"=>array($query)), false, Array(), array("ID","NAME"));
		$arId = [];
		...
		/*$arFilter[] = array( 
			"LOGIC" => "OR", 
				 array("QUERY" =>  $arResult["REQUEST"]["~QUERY"]), 
				 array("=QUERY" => $query) 
		 ); */
		 $arFilter["SITE_ID"] = SITE_ID;
		 $arFilter["TAGS"] = $arResult["REQUEST"]["~TAGS"];
		...
		if($obSearch->errorno==0)
		{
			...
			while($ar)
			{
				...
				$obItemProps = CIBlockElement::GetProperty(16, $ar["ITEM_ID"], ["sort"=>"asc"], ['CODE' => 'CROSS_NUMBERS']);
					...
					{
						$arItemProps[$ar["ITEM_ID"]][] = (string)strtoupper($arItemProp['VALUE']);
					}
					$res = CIBlockElement::GetByID($ar['ITEM_ID']);
					if($ar_res = $res->GetNext())
						$sName = $ar_res['NAME'];	
					$str = substr_count(mb_strtoupper($sName),mb_strtoupper($q));
				if ( !$str && !checkCrossNumber1($arItemProps[$ar["ITEM_ID"]],$query))
					{
						unset($arReturn[$ar["ID"]]);
					}
				$arResult["SEARCH"][]=$ar;
				$ar = $obSearch->GetNext(); 
			}
